...
export default function EmergencyContacts() {
  ...
  const handleAddContact = async () => {
    if (newContact.name && newContact.number) {
      ...
      setCustomContacts([...customContacts, { id: docRef.id, ...newContact }]);
      setNewContact({ name: "", number: "" });
      setIsAddingContact(false);
    }
  };
  ...
  return (
    <div className="bg-gradient-to-b from-gray-50 to-gray-200 min-h-screen p-6 mt-10">
      ...
      <div className="max-w-2xl mx-auto">         {/* 🚨 Emergency Contacts */}
        <h1 className="text-4xl font-extrabold text-gray-800 text-center mb-6">🚨 Emergency Contacts</h1>
       <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
           {emergencyContacts.map((contact) => (
            <motion.button
        ...
        <h1 className="text-4xl font-extrabold text-gray-800 text-center mb-6">📞 Custom Contacts</h1>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {customContacts.map((contact) => (
            <motion.button
              key={contact.id}
